...
1. **[カスタム デプロイ]** ブレードで、次の設定を指定します (他の設定は既定値のままにします)。
    ...
    | Virtual Machine Name、Virtual Machine Name1、Virtual Machine Computer Name1   | **az104-08-vm1** |
    | Virtual Machine RG | **az104-08-rg01** |    
    | Admin Username | **Student** |
    | Admin Password | **セキュリティで保護されたパスワードを指定する**  |
    | Enable Hotpatching | **false** |
    | Zone | **2** |

    >**注**:仮想マシンとそのネットワーク インターフェイスを含む、テンプレートを使用して、デプロイしている個別のリソースのプロパティに対応するパラメーターを変更する必要があります。

1. **[確認と作成]** をクリックし、**[確認と作成]** ブレードで **[作成]** をクリックします。
...
1. **[実行コマンドスクリプト]** ブレードで次のコマンドを入力し、**[実行]** をクリックして **az104-08-vm0** でホストされている Web サイトにアクセスします。
...
1. **[仮想マシン スケール セットの作成]** ブレードの **[ネットワーク]** タブに戻り、**[負荷分散のオプション]** セクションで、**[Azure Load Balancer]** を選択し、**[ロード バランサーの作成]**　を選択し、以下の設定で作成します。 (他の設定は既定値のままにします)

    | 設定 | 値 |
    | --- | --- |
    | ロード バランサー名 | **(新規) az10408vmss0-lb** |
...
1. **[az10408vmss0]** ブレードの **[設定]** セクションで、**[拡張機能とアプリケーション]** をクリックし、**[追加]** をクリックします。

1. **[新しいリソース]** ブレードで、**[Custom Script Extension]** 、**[次へ]** の順にクリックします。
...
1. 使用可能なサイズのリストで、**[Standard DS1_v2]** を選択し、**[サイズ変更]** をクリックします。
